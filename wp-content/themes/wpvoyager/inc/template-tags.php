...
if ( ! function_exists( 'wpvoyager_add_menuid' ) ) :
	function wpvoyager_add_menuid ($page_markup) {
	...
	add_filter('wp_page_menu', 'wpvoyager_add_menuid');
endif;
...
function wpvoyager_posted_on() {
  ...
  if(is_single()) {
    ...
    if (in_array("author", $metas)) {
        ...
        echo '<i class="fa fa-user"></i>'. __('By','wpvoyager'). ' <a class="author-link" itemprop="url" rel="author" href="'.esc_url(get_author_posts_url(get_the_author_meta('ID' ))).'">'; the_author_meta('display_name'); echo'</a>';
        echo '</span>';
    }
    ...
    if (in_array("com", $metas)) {
      echo '<span><i class="fa fa-comment"></i>'; comments_popup_link( __('With 0 comments','wpvoyager'), __('With 1 comment','wpvoyager'), __('With % comments','wpvoyager'), 'comments-link', __('Comments are off','wpvoyager')); echo '</span>';
    }
  } else {
...
function wpvoyager_full_posted_on() {
	...
	$byline = '<span class="author vcard">'.__('By','wpvoyager').' <a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>';
...
if ( ! function_exists( 'wpv_related_posts' ) ) :
	function wpv_related_posts($post) {
		...
		if($status == 'on'){
		    ...
		    echo '<div class="related-posts col-'. ot_get_option('pp_related_columns',3).'">';
		    if ($categories) {
		        $category_ids = array();
		        ...
		        $args=array(
		            ...
		            'posts_per_page'=> ot_get_option('pp_related_columns',3), // Number of related posts that will be shown.
  ...
  function dimox_breadcrumbs() {
    ...
    if (is_home() || is_front_page()) {
      ...
      if ( is_category() ) {
        ...
        if ($thisCat->parent != 0) $output .= '<li>'.get_category_parents($thisCat->parent, TRUE, ' ' . $delimiter . ' ');
  ...
  <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
       ...
       <div class="comment-content"><div class="arrow-comment"></div>
            <div class="comment-by"><?php printf( '<strong>%s</strong>', get_comment_author_link() ); ?>  <span class="date"> <?php printf( __( '%1$s at %2$s', 'wpv' ), get_comment_date(), get_comment_time() ); ?></span>
               <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __('<i class="fa fa-reply"></i> Reply','wpvoyager'), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
            </div>
            <?php comment_text(); ?>
