...
get_header(); 
$layout = ot_get_option('pp_archive_layout','full-width');
$position = 'alternative';
$mapflag = true;
if(is_tag()) { 
	$tag_ID = get_query_var('tag_id'); 
	$category_settings = get_option("category_$tag_ID"); 
} 
...
if(isset($category_settings['map'])) {
	...
	if ($map_type == 'none') {
		...
		echo do_shortcode('[tp-global-map class="'.$position.'" tag="'.$tag_ID.'"]' );
	} else {
...
<div class="container <?php echo esc_attr($layout); ?>">
	...
	<?php } else { ?>
		<div class="sixteen columns">
	<?php } 
		$post_style = ot_get_option('pp_archive_posts_style','std');
 			if ($post_style=='std') {
 				$post_style = '';
 			}
 			if ( have_posts() ) :
