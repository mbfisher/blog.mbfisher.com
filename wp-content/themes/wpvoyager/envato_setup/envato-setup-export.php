...
$post_types      = array( 'attachment', 'wpcf7_contact_form', 'post', 'page', 'portfolio', 'testimonial', 'team' );
...
//			print_r($meta); exit;
		...
		foreach ( $meta as $meta_key => $meta_val ) {
			if (
				...
				(
					...
					strpos( $meta_key, 'incr_' ) === false &&
					strpos( $meta_key, 'centum_' ) === false &&
		...
		if ( $post_type == 'attachment' ) {
			...
			if ( is_file( $file ) ) {
				...
				if ( is_dir( get_home_path() . '/wp-content/uploads/centum/images/' ) ) {
					copy( $file, get_home_path() . '/wp-content/uploads/centum/images/' . basename( $file ) );
				}
			}
...
foreach ( $menus as $menu ) {
	if ( $menu->name == 'menu' ) {
		$menu_ids['mainmenu'] = $menu->term_id;
	} 
}
...
if ( is_dir( get_home_path() . '/wp-content/uploads/centum/' ) ) {
	echo "it exists";
	file_put_contents( get_home_path() . '/wp-content/uploads/centum/default.json' , json_encode( $default_content ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/centum/widget_positions.json' , json_encode( $widget_positions ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/centum/widget_options.json' , json_encode( $widget_options ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/centum/menu.json' , json_encode( $menu_ids ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/centum/options.json' , json_encode( $my_options ) );
}
